...
import {
  ...
  ChevronUp, Download, Eye, FileText, Loader2, MessageSquare, Search,
  Sparkles, Star, TrendingUp, Trophy, Users, XCircle,
} from "lucide-react"
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          ...
          return (
            <Card key={item.label} className="border-border/70 bg-card shadow-sm">
              <CardContent className="flex items-center gap-4 p-5">
                <div className={`flex h-11 w-11 items-center justify-center rounded-xl ${item.box}`}><Icon className={`h-5 w-5 ${item.text}`} /></div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <Card className="border-border/70 bg-card shadow-sm">
          <CardHeader><CardTitle className="flex items-center gap-2 text-base text-foreground"><BarChart3 className="h-4 w-4 text-primary" /> Job Status Distribution</CardTitle></CardHeader>
        ...
        <Card className="border-border/70 bg-card shadow-sm">
          <CardHeader><CardTitle className="flex items-center gap-2 text-base text-foreground"><Sparkles className="h-4 w-4 text-primary" /> Most Requested Skills</CardTitle></CardHeader>
          <CardContent className="px-6 pb-6">
            {topSkills.length ? <MiniBars data={topSkills} height={148} /> : <p className="text-sm text-muted-foreground">Post jobs with required skills to unlock demand analytics.</p>}
          </CardContent>
        </Card>
      </div>
      <Card className="border-border/70 border-l-4 border-l-primary bg-card shadow-sm">
        <CardContent className="flex items-start gap-3 p-5">
          <Sparkles className="mt-0.5 h-5 w-5 shrink-0 text-primary" />
          <p className="text-sm text-muted-foreground">Select any posted job below to inspect candidate fit, shortlist top applicants, and export recruiter-ready reports.</p>
        </CardContent>
      </Card>
    </div>
  )
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          { label: "Avg AI Score", value: `${avg}%`, icon: TrendingUp, color: "text-yellow-400", bg: "bg-yellow-500/10" },
          { label: "Top Score", value: `${top}%`, icon: Trophy, color: "text-green-400", bg: "bg-green-500/10" },
          { label: "Filter Pass", value: `${passed}/${total}`, icon: CheckCircle2, color: "text-blue-400", bg: "bg-blue-500/10" },
        ].map(item => {
          const Icon = item.icon
          return (
            <Card key={item.label} className="border-border/70 bg-card shadow-sm">
              <CardContent className="flex items-center gap-3 p-4">
                <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${item.bg}`}><Icon className={`h-5 w-5 ${item.color}`} /></div>
                <div><p className={`text-xl font-bold ${item.color}`}>{item.value}</p><p className="text-xs text-muted-foreground">{item.label}</p></div>
              </CardContent>
            </Card>
          )
        })}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card className="border-border/70 bg-card shadow-sm"><CardHeader className="pb-2"><CardTitle className="text-sm text-foreground">Score Distribution</CardTitle></CardHeader><CardContent className="px-4 pb-4"><MiniBars data={[{ label: "0-25", value: scores.filter(s => s <= 25).length, color: "bg-red-500" }, { label: "26-50", value: scores.filter(s => s > 25 && s <= 50).length, color: "bg-orange-500" }, { label: "51-75", value: scores.filter(s => s > 50 && s <= 75).length, color: "bg-yellow-500" }, { label: "76-100", value: scores.filter(s => s > 75).length, color: "bg-green-500" }]} /></CardContent></Card>
        <Card className="border-border/70 bg-card shadow-sm"><CardHeader className="pb-2"><CardTitle className="text-sm text-foreground">Hard Filter Result</CardTitle></CardHeader><CardContent className="flex flex-col items-center gap-2 pb-4"><Donut pass={passed} fail={failed} /></CardContent></Card>
        <Card className="border-border/70 bg-card shadow-sm"><CardHeader className="pb-2"><CardTitle className="text-sm text-foreground">Bonus Breakdown</CardTitle></CardHeader><CardContent className="px-4 pb-4"><MiniBars data={[{ label: "Skill", value: Math.round(totals.skill), color: "bg-primary" }, { label: "Elite", value: Math.round(totals.intern), color: "bg-blue-500" }, { label: "Project", value: Math.round(totals.project), color: "bg-violet-500" }, { label: "Duration", value: Math.round(totals.duration), color: "bg-cyan-500" }]} /></CardContent></Card>
      </div>
    </div>
  )
})

const CandidateCard = memo(function CandidateCard({ candidate, index, recommended, job, onShortlist, onMessage }: { candidate: RankedCandidate; index: number; recommended: boolean; job: Job | null; onShortlist: (c: RankedCandidate) => void; onMessage: (c: RankedCandidate) => void }) {
  ...
  const final = pct(candidate.final_score_d)
  ...
  return (
    <>
      <Card
        className={`bg-card border-border transition-colors hover:border-primary/40 ${recommended ? "ring-2 ring-primary/20" : ""}`}
        style={WINDOWED_CANDIDATE_STYLE}
      >
        <CardContent className="p-5">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center">
            <div className="flex min-w-[220px] items-center gap-3">
              <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl font-bold ${recommended ? "bg-primary/20 text-primary" : "bg-secondary text-muted-foreground"}`}>#{candidate.rank || index + 1}</div>
              <Avatar className="h-12 w-12 border-2 border-border"><AvatarFallback className="bg-primary/10 font-semibold text-primary">{candidate.full_name.split(" ").map(n => n[0]).join("").slice(0, 2)}</AvatarFallback></Avatar>
              <div>
                <div className="flex flex-wrap items-center gap-1.5"><span className="text-sm font-semibold text-foreground">{candidate.full_name}</span>{recommended && <Badge className="border-0 bg-primary/10 text-xs text-primary">Recommended</Badge>}</div>
                <p className="text-xs text-muted-foreground">CGPA {candidate.cgpa ?? "-"}</p>
              </div>
            </div>

            <div className="flex min-w-[90px] items-center gap-2">
              <Sparkles className="h-4 w-4 shrink-0 text-primary" />
              <div><p className="text-xs text-muted-foreground">AI Score</p><p className={`text-2xl font-bold ${scoreColor(final)}`}>{final}%</p></div>
            </div>
              ...
              {barSets[barMode].map(row => (
                <div key={row.label} className="flex items-center gap-2">
                  ...
                  <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-secondary">
                    <div className={`h-full ${barColor(row.value)}`} style={{ width: `${row.value}%` }} />
                  </div>
                  <span className="w-9 text-right text-[11px] text-muted-foreground">{row.value}%</span>
                </div>
              ))}
            </div>

            <div className="hidden flex-1 2xl:block"><p className="line-clamp-3 text-xs italic text-muted-foreground">"{explain}"</p></div>

            <div className="flex shrink-0 items-center gap-2">
              <Badge className={`border-0 text-xs ${candidate.status === "rejected" ? "bg-red-500/10 text-red-400" : candidate.status === "shortlisted" ? "bg-primary/10 text-primary" : candidate.status === "scored" ? "bg-green-500/10 text-green-400" : "bg-secondary text-muted-foreground"}`}>{candidate.status}</Badge>
              <Button variant="outline" size="sm" onClick={() => setOpen(true)} className="h-8 w-8 border-border p-0 text-muted-foreground hover:text-foreground"><Eye className="h-3.5 w-3.5" /></Button>
              <Button size="sm" onClick={() => onShortlist(candidate)} className="h-8 border border-primary/20 bg-primary/10 px-2 text-xs text-primary hover:bg-primary/20"><CheckCircle2 className="mr-1 h-3.5 w-3.5" /> Shortlist</Button>
            </div>
          </div>
          {candidate.passed_hard_filter === false && candidate.filter_fail_reason && <div className="mt-3 flex items-center gap-2 rounded-lg bg-red-500/10 p-2 text-xs text-red-400"><AlertCircle className="h-3.5 w-3.5 shrink-0" />{candidate.filter_fail_reason}</div>}
        </CardContent>
      </Card>
        ...
        <DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto border-border bg-card">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3 text-foreground">
              <Avatar className="h-11 w-11 border-2 border-border"><AvatarFallback className="bg-primary/10 font-semibold text-primary">{candidate.full_name.split(" ").map(n => n[0]).join("").slice(0, 2)}</AvatarFallback></Avatar>
              <div><div className="flex items-center gap-2">{candidate.full_name}{recommended && <Badge className="border-0 bg-primary/10 text-xs text-primary">Recommended</Badge>}</div><p className="text-sm font-normal text-muted-foreground">{candidate.college || "Candidate"}</p></div>
            </DialogTitle>
            <DialogDescription>Applied on {fmtDate(candidate.applied_at)}</DialogDescription>
          </DialogHeader>
          <div className="mt-2 space-y-5">
            <div className="rounded-lg border border-primary/20 bg-primary/5 p-4"><div className="mb-2 flex items-center gap-2"><Sparkles className="h-4 w-4 text-primary" /><span className="text-sm font-medium text-foreground">AI Assessment</span></div><p className="text-sm italic text-muted-foreground">"{explain}"</p></div>
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              <div className="space-y-3 rounded-lg border border-border/60 bg-secondary/30 p-4">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium text-foreground">Assessment Bars</p>
                  <div className="flex items-center gap-1">
                ...
                {barSets[barMode].map(row => (
                  <div key={row.label} className="flex items-center gap-2">
                    ...
                    <span className={`w-10 text-right text-sm font-semibold ${scoreColor(row.value)}`}>{row.value}%</span>
                  </div>
                ))}
              </div>
              <div className="rounded-lg border border-border/60 bg-secondary/30 p-4">
                <p className="mb-3 text-sm font-medium text-foreground">Bonus Signals</p>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <div key={label as string} className="rounded-md bg-secondary/60 p-2.5">
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              <div className="rounded-lg bg-secondary/30 p-4"><p className="mb-3 text-sm font-medium text-foreground">Matched Skills</p>{matched.length ? <div className="flex flex-wrap gap-2">{matched.map(skill => <Badge key={skill} className="border-0 bg-green-500/10 text-green-400">{skillLabel(skill)}</Badge>)}</div> : <p className="text-sm text-muted-foreground">No explicit skill match signal extracted yet.</p>}</div>
              <div className="rounded-lg bg-secondary/30 p-4"><p className="mb-3 text-sm font-medium text-foreground">Missing Target Skills</p>{missing.length ? <div className="flex flex-wrap gap-2">{missing.map(skill => <Badge key={skill} className="border-0 bg-red-500/10 text-red-400">{skillLabel(skill)}</Badge>)}</div> : <p className="text-sm text-muted-foreground">No major gaps detected against listed job skills.</p>}</div>
            </div>
            ...
            ].map(({ Icon, label, value }) => <div key={label} className="flex items-center gap-3 rounded-lg bg-secondary/30 p-3"><Icon className="h-4 w-4 shrink-0 text-muted-foreground" /><div><p className="text-xs text-muted-foreground">{label}</p><p className="text-sm text-foreground">{String(value)}</p></div></div>)}</div>
            {candidate.filter_fail_reason && <div className="flex items-center gap-2 rounded-lg border border-red-500/20 bg-red-500/10 p-3 text-sm text-red-400"><XCircle className="h-4 w-4 shrink-0" />{candidate.filter_fail_reason}</div>}
            <div className="flex gap-2 border-t border-border pt-2"><Button onClick={() => { onShortlist(candidate); setOpen(false) }} className="flex-1 bg-primary text-sm text-primary-foreground"><CheckCircle2 className="mr-2 h-4 w-4" /> Shortlist</Button><Button variant="outline" className="flex-1 border-border text-sm text-foreground hover:bg-secondary" onClick={() => { onMessage(candidate); setOpen(false) }}><MessageSquare className="mr-2 h-4 w-4" /> Message</Button></div>
          </div>
        </DialogContent>
      </Dialog>
    </>
      ...
      <div className="space-y-6">
        ...
        <Card className="bg-card border-border">
          <CardContent className="space-y-4 p-5">
            <div className="flex items-center gap-3"><Search className="h-4 w-4 shrink-0 text-muted-foreground" /><Input value={jobSearch} onChange={e => setJobSearch(e.target.value)} placeholder="Search job by title..." className="border-border bg-input text-foreground" /></div>
            {filteredJobs.length === 0 ? <p className="py-6 text-center text-sm text-muted-foreground">No jobs found.</p> : <div className="space-y-2">{filteredJobs.map(job => <button key={job.id} onClick={() => selectJob(job)} className="flex w-full items-center justify-between rounded-xl border border-border bg-secondary/20 p-4 text-left transition-colors hover:border-primary/50 hover:bg-secondary/40"><div><div className="flex items-center gap-2">{unseenApplicationJobIds.has(job.id) && <span className="inline-flex h-2.5 w-2.5 rounded-full bg-emerald-500 shadow-[0_0_0_3px_rgba(16,185,129,0.16)]" />}<p className="text-sm font-medium text-foreground">{job.title}</p></div><p className="mt-0.5 text-xs text-muted-foreground">{job.required_skills?.length ? `${job.required_skills.length} target skills defined` : "Click to view candidate ranking"}</p></div><Badge className={`border-0 text-xs ${job.status === "active" ? "bg-green-500/10 text-green-400" : job.status === "paused" ? "bg-yellow-500/10 text-yellow-400" : "bg-secondary text-muted-foreground"}`}>{job.status ?? "active"}</Badge></button>)}</div>}
          </CardContent>
        </Card>
      </div>
    )
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-14 sm:pt-0">
        <div className="sm:pl-24">
          <h2 className="font-semibold text-foreground">{selectedJob.title}</h2>
          <p className="text-xs text-muted-foreground">{totalCandidates} applicants</p>
        </div>
        <div className="flex items-center gap-2"><Button variant="outline" size="sm" onClick={() => exportCSV(candidates, selectedJob.title)} className="border-border text-xs text-muted-foreground hover:text-foreground"><Download className="mr-1 h-3.5 w-3.5" /> CSV</Button><Button variant="outline" size="sm" onClick={() => exportPDF(candidates, selectedJob.title)} className="border-border text-xs text-muted-foreground hover:text-foreground"><FileText className="mr-1 h-3.5 w-3.5" /> PDF</Button><Button size="sm" onClick={() => handleShortlistCandidate(null)} className="bg-primary text-xs text-primary-foreground"><Star className="mr-1 h-3.5 w-3.5" /> Shortlist</Button></div>
      </div>
      {!loading && candidates.length > 0 && <JobAnalytics candidates={candidates} />}
      <Card className="bg-card border-border"><CardContent className="p-4"><div className="flex flex-wrap items-center gap-3"><div className="relative min-w-[220px] flex-1"><Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><Input value={candSearch} onChange={e => setCandSearch(e.target.value)} placeholder="Search candidates..." className="h-9 border-border bg-input pl-9 text-sm text-foreground" /></div><Select value={filterStatus} onValueChange={setFilterStatus}><SelectTrigger className="h-9 w-36 border-border bg-input text-sm text-foreground"><SelectValue placeholder="Status" /></SelectTrigger><SelectContent className="border-border bg-card"><SelectItem value="all">All Status</SelectItem><SelectItem value="scored">Scored</SelectItem><SelectItem value="shortlisted">Shortlisted</SelectItem><SelectItem value="rejected">Rejected</SelectItem><SelectItem value="pending">Pending</SelectItem></SelectContent></Select><Select value={filterPassed} onValueChange={setFilterPassed}><SelectTrigger className="h-9 w-36 border-border bg-input text-sm text-foreground"><SelectValue placeholder="Filter" /></SelectTrigger><SelectContent className="border-border bg-card"><SelectItem value="all">All Candidates</SelectItem><SelectItem value="passed">Passed Filter</SelectItem><SelectItem value="failed">Failed Filter</SelectItem></SelectContent></Select><Button variant="outline" size="sm" onClick={() => setSortOrder(cur => cur === "desc" ? "asc" : "desc")} className="h-9 border-border text-sm text-muted-foreground hover:text-foreground">{sortOrder === "desc" ? <ChevronDown className="mr-1 h-3.5 w-3.5" /> : <ChevronUp className="mr-1 h-3.5 w-3.5" />}{sortOrder === "desc" ? "Highest First" : "Lowest First"}</Button><p className="ml-auto text-xs text-muted-foreground">Showing {candidates.length} of {totalCandidates}</p></div></CardContent></Card>
      {loading ? <div className="flex justify-center py-16"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : candidates.length === 0 ? <Card className="bg-card border-border"><CardContent className="space-y-3 py-16 text-center"><Users className="mx-auto h-10 w-10 text-muted-foreground" /><p className="font-medium text-foreground">No candidates found</p><p className="text-sm text-muted-foreground">Try adjusting your filters</p></CardContent></Card> : <div className="space-y-3">{candidates.map((candidate, index) => <CandidateCard key={candidate.application_id} candidate={candidate} index={index} recommended={index < Math.max(selectedJob.vacancies ?? 3, 3)} job={selectedJob} onShortlist={handleShortlistCandidate} onMessage={handleMessage} />)}<div ref={loadMoreRef} className="h-2 w-full" />{loadingMore && <div className="flex justify-center py-4"><Loader2 className="h-5 w-5 animate-spin text-primary" /></div>}</div>}
      {shortlistOpen && (
        <ShortlistDialog
          open={shortlistOpen}
